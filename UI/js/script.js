...
function accept_loan_function2(object) {
    var loan_id = object.name;

    var tmp = document.getElementById('check_request_view_list');
    ...
    $.ajax({
        ...
        url: '/Controller',
        data: {
            what: 10, // 9 : give me a request list
            loan_id: loan_id
        },
        success: function(data) {
            $.ajax({
                ...
                url: '/Controller',
                data: {
                    what: 9, // 9 : give me a request list
                    id: current_email
                },
                success: function(data) {
                    var data = JSON.parse(data);

                    for (var i = 0; i < data.credit_info.length; i ++) {

                        var div = document.createElement("div");
                        var content = '';
                        content += '<div id="' + data.credit_info[i].loans_id + '" class="col-md-12 borrow_money_info"';
                        content += '<h4><span id="receiver">' + data.credit_info[i].debtor_name + '</span> 님의 요청</h4>';
                        content += '<span id="limit_date">' + data.credit_info[i].finish_date + '</span> &nbsp;까지<br>';
                        content += '<span id="receiver_to_sender_money_all">' + data.credit_info[i].total_money + '</span> &nbsp;원<br>';
                        content += '</div>';
                        content += '<button id="accept_loan" name="' + data.credit_info[i].loans_id + '" type="button" onclick="accept_loan_function2(this)" class="btn btn-success">수락</button>';
                        content += '<button id="reject_loan" name="' + data.credit_info[i].loans_id + '" type="button" onclick="reject_loan_function2(this)" class="btn btn-danger">거절</button>';
                        div.innerHTML = content;

                        document.getElementById('check_request_view_list').appendChild(div);
                    }
...
function reject_loan_function2(object) {
    var loan_id = object.name;
...
function delete_loan2(object) {
    var load_id = document.getElementById('delete_loan').name;

    while (document.getElementById('borrow_content').hasChildNodes()){
        document.getElementById('borrow_content').removeChild(document.getElementById('borrow_content').firstChild);
    }
    while (document.getElementById('loan_content').hasChildNodes()){
        document.getElementById('loan_content').removeChild(document.getElementById('loan_content').firstChild);
    }
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            for (var i = 0; i < data.info[0].credit_info.length; i ++) {
                ...
                content += '<div class="progress-bar" role="progressbar" aria-valuenow="70" aria-valuemin="0" aria-valuemax="100" style="width:' + percent + '%"></div></div><button id="delete_loan" name="' + data.info[0].credit_info[i].loans_id + '" type="button" onclick="delete_loan2(this)" class="btn btn-danger">&nbsp;<span class="glyphicon glyphicon-remove" aria-hidden="true"></span>&nbsp;</button></div>';
                div.innerHTML = content;
            ...
            for (var i = 0; i < data.info[1].debt_info.length; i ++) {
                ...
                content += '<span id="receiver_to_sender_money_current">' + data.info[1].debt_info[i].balance_money + '</span>&nbsp; 원&nbsp;&nbsp;/&nbsp;&nbsp;<span id="receiver_to_sender_money_all">' + data.info[1].debt_info[i].total_money + '</span> &nbsp;원<br>';
                content += '<div class="progress">';
                ...
                content += '<button id="send_money_button" name="' + data.info[1].debt_info[i].loans_id + '" type="button" onclick="send_money_function2(this)" class="btn btn-danger send_money_button">갚기</button>';
...
function send_money_function2(object){
    addClassName(document.getElementById('send_money_button'),"disable");
    removeClassName(document.getElementById('real_send'),"disable");
}


function give_me_the_money_friend_email_function(object) {
    object.setAttribute("style", "background-color:#8ad1ff;");
    give_me_the_money_friend_name = object.innerText;
    give_me_the_money_friend_email = object.getAttribute("value");
}

function send_real_money_function2(object,money) {
    // object.target.name -> loans_id
    var loans_id = object.name;
    var money = 100;
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                for (var i = 0; i < data.info[1].debt_info.length; i ++) {
                    ...
                    content += '<div id="real_send" class="input-group disable">';
                    content += '<input id="send_money_input" type="text" class="form-control" placeholder="갚을 금액">';
                    content += '<span class="input-group-btn"><button id="real_send_money_button" name="' + data.info[1].debt_info[i].loans_id + '" onclick="send_real_money_function2(this)" class="btn btn-danger" type="button">보내기!</button></span>';
                    content += '</div></div>';
                    div.innerHTML = content;
                    document.getElementById('loan_content').appendChild(div);
                }
...
function Fincnic() {
    ...
    this.check_request_menu = document.getElementById('check_request_menu');
    this.check_request_menu.addEventListener('click',this.check_request_content.bind(this));
    this.footer_check_request_menu = document.getElementById('footer_check_request_menu');
    this.footer_check_request_menu.addEventListener('click',this.check_request_content.bind(this));

    this.check_request_view = document.getElementById('check_request_view');

    this.accept_loan = document.getElementById('accept_loan');
    this.accept_loan.addEventListener('click',this.accept_loan_function.bind(this));

    this.reject_loan = document.getElementById('reject_loan');
    this.reject_loan.addEventListener('click',this.reject_loan_function.bind(this));

}

Fincnic.prototype.accept_loan_function = function() {
    var loan_id = this.accept_loan.name;
...
Fincnic.prototype.reject_loan_function = function() {
    var loan_id = this.reject_loan.name;
...
Fincnic.prototype.check_request_content = function() {
    ...
    removeClassName(this.check_request_view,'disable');
...
Fincnic.prototype.logout_function = function() {
    ...
    addClassName(this.check_request_view,'disable');
...
Fincnic.prototype.show_friend_list = function() {
    ...
    while (document.getElementById('friend_list').hasChildNodes()){
        document.getElementById('friend_list').removeChild(document.getElementById('friend_list').firstChild);
    }
...
Fincnic.prototype.show_add_account = function() {
    ...
    while(document.getElementById('account_list').hasChildNodes()){
        document.getElementById('account_list').removeChild(document.getElementById('account_list').firstChild);
    }
...
Fincnic.prototype.show_me_the_money_content = function() {
    ...
    while (document.getElementById('my_friend_list').hasChildNodes()){
        document.getElementById('my_friend_list').removeChild(document.getElementById('my_friend_list').firstChild);
    }
...
Fincnic.prototype.give_me_the_money_function = function() {
    var money = document.getElementById('moneymoney').value;
